...
public class PredefinedStepDefinitions implements BaseTest {
	...
	@Then("^I should be able to see element having (.+) as \"([^\"]*)\" should\\s*((?:not)?)\\s+be present have partial text as \\\"(.*?)\\\"$\"$")
	public void iShouldBeAbleToSeeElementHavingIdAsF(String type,String accessName,String present) throws Throwable {
	}

	@Then("^I should be able to see element having (.+) as \"([^\"]*)\" should\\s*((?:not)?)\\s+have text as \"([^\"]*)\"$")
	public void iShouldBeAbleToSeeElementHavingIdAsShouldHaveTextAs(String type,String accessName,String present,String value) throws Throwable {
		//progressObj.waitForElementToDisplay(type, accessName, "1");
		Thread.sleep(1000);
		miscmethodObj.validateLocator(type);
		assertionObj.checkElementPartialText(type, value, accessName, present.isEmpty());
	}
